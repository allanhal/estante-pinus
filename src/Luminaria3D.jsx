...
import { ESPESSURA, LARGURA_PECA, getSupportRun } from "./Luminaria";
...
const SOQUETE_MATERIAL = new THREE.MeshStandardMaterial({
  color: 0x8d8880,
  roughness: 0.5,
  metalness: 0.1,
});
const ROSCA_MATERIAL = new THREE.MeshStandardMaterial({
  color: 0xb8b3aa,
  roughness: 0.35,
  metalness: 0.8,
});
const LAMPADA_MATERIAL = new THREE.MeshStandardMaterial({
  color: 0xffd23f,
  emissive: 0xffc107,
...
function Luminaria3D({ height = 30, depth = 20 }) {
  ...
  useEffect(() => {
    ...
    const scene = new SceneInit("luminariaThreeJsCanvas");
    scene.initialize();
  ...
  useEffect(() => {
    ...
    // A altura informada é a total: o wall vai do chão da peça até a base do top.
    const wallComprimento = height - ESPESSURA;
    // O top começa colado na alvenaria e avança a profundidade pedida.
    const topFim = depth - ESPESSURA;

    // Peça "WALL": costa vertical parafusada na alvenaria (x de -ESPESSURA a 0).
    // Termina embaixo do top, que apoia sobre o topo dela.
    const addWall = () => {
    ...
    // Peça "TOP": braço horizontal deitado sobre o wall, encostando na alvenaria.
    const addTop = () => {
      const geometry = new THREE.BoxGeometry(depth, ESPESSURA, width);
      addMeshWithEdges(geometry, [-ESPESSURA + depth / 2, bracoY, 0]);
    };

    // Peça "SUPPORT": mão-francesa a 45° entre o wall e a face inferior do top.
    // As pontas são cortadas em esquadria (vertical na face do wall, horizontal
    // sob o top) para encostarem sem invadir as outras peças.
    const addSupport = () => {
      const topBottom = height - ESPESSURA;
      const base = topBottom - supportRun;
      // Deslocamento vertical equivalente à espessura medida na perpendicular.
      ...
      // O extrude cresce em +z; recentraliza na largura da peça.
      geometry.translate(0, 0, -width / 2);

      addMeshWithEdges(geometry, [0, 0, 0]);
    };

    const soqueteX = Math.max(topFim - 3, ESPESSURA + 3);
    const soqueteTopo = height - ESPESSURA;
    // Medidas reais do padrão E27, em cm: rosca Ø2.6 (26.05–26.45mm) e corpo do
    // bocal Ø3.9. A lâmpada é uma A60, Ø6.
    const ROSCA_RAIO = 1.3;
    const ROSCA_ALTURA = 1.6;
    const BOCAL_RAIO = 1.95;
    const BOCAL_ALTURA = 4;
    const lampadaRaio = 3;

    const addSoqueteELampada = () => {
      // Corpo do bocal, preso na face de baixo do top.
      const bocalY = soqueteTopo - BOCAL_ALTURA / 2;
      addMeshWithEdges(
        new THREE.CylinderGeometry(BOCAL_RAIO, BOCAL_RAIO, BOCAL_ALTURA, 24),
        [soqueteX, bocalY, 0],
        0,
        SOQUETE_MATERIAL
      );

      // Rosca E27 entre o bocal e a lâmpada.
      const roscaY = soqueteTopo - BOCAL_ALTURA - ROSCA_ALTURA / 2;
      addMeshWithEdges(
        new THREE.CylinderGeometry(ROSCA_RAIO, ROSCA_RAIO, ROSCA_ALTURA, 20),
        [soqueteX, roscaY, 0],
        0,
        ROSCA_MATERIAL
      );

      // Lâmpada A60 redonda (sem arestas, pra ficar lisa). Sobe um pouco para o
      // bulbo abraçar a rosca, como na peça real.
      const lampadaY = soqueteTopo - BOCAL_ALTURA - ROSCA_ALTURA - lampadaRaio + 0.8;
      const lampada = new THREE.Mesh(
        new THREE.SphereGeometry(lampadaRaio, 32, 24),
        LAMPADA_MATERIAL
      );
      lampada.position.set(soqueteX, lampadaY, 0);
      modelGroup.add(lampada);

      const luz = new THREE.PointLight(0xffe0a3, 0.8, 200);
      luz.position.set(soqueteX, lampadaY, 0);
      modelGroup.add(luz);
    };
    ...
    const exportBtn = document.getElementById("button-luminaria");
    ...
    const handleExport = () => {
      ...
      link.download = "luminaria-pinus.stl";
      link.click();
  ...
  }, [width, height, depth]); // width é constante, mantido para clareza do modelo

  return (
    <div className="w-full h-full relative">
      <canvas id="luminariaThreeJsCanvas" className="w-full h-full block" />
    </div>
  );
}

export default Luminaria3D;

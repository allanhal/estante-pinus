...
import Luminaria3D from "./Luminaria3D";
import LuminariaControls from "./LuminariaControls";
...
// Soquete, cabo, plugue, interruptor e lâmpada refletora.
export const KIT_ELETRICO = 45;
const LUMINARIA_MARKUP = 1.2;
...
export const calculateLuminariaBillOfMaterials = ({ height, depth }) => [
  // O top apoia sobre o wall, então a altura total desconta a espessura da ripa.
  { nome: "Wall", comprimento: height - ESPESSURA, quantidade: 1 },
  { nome: "Top", comprimento: depth, quantidade: 1 },
  { nome: "Suporte", comprimento: getSupportComprimento(height, depth), quantidade: 1 },
];

export const calculateLuminariaPrice = ({ height, depth }) => {
  const totalLength = calculateLuminariaBillOfMaterials({ height, depth }).reduce(
    (sum, item) => sum + item.comprimento * item.quantidade,
    0
  );
  const material = (totalLength / 100) * PRECO_POR_METRO;

  return Math.round((material * 2 + KIT_ELETRICO) * LUMINARIA_MARKUP);
};
    ...
    <div className="h-screen lg:h-auto lg:min-h-screen bg-[var(--background)] flex flex-col overflow-hidden lg:overflow-visible transition-colors duration-300">
      ...
      <main className="flex-1 flex flex-col lg:grid lg:grid-cols-12 gap-0 lg:gap-8 lg:px-12 pb-0 lg:pb-12 overflow-hidden lg:overflow-visible">
        ...
        <div className="h-[40vh] shrink-0 lg:col-span-7 lg:sticky lg:top-8 lg:h-[80vh] bg-white dark:bg-stone-900 lg:rounded-3xl shadow-2xl lg:shadow-amber-900/10 dark:lg:shadow-black/30 overflow-hidden group lg:self-start transition-colors duration-300">
          <Luminaria3D height={height} depth={depth} />

...
const calculateFrete = (quantity) => {
  const tierMultiplier = 1 + Math.floor((quantity - 1) / 10) * 0.5;

  return Math.round(FRETE_FIXO_MOTO * tierMultiplier);
};

const MaoFrancesaControls = ({
  height,
  depth,
  quantity,
  showBom,
  ...
  const instalacaoValue = CUSTO_INSTALACAO * quantity;
  const freteValue = calculateFrete(quantity);
  const totalValue =
    price +
    (includeInstalacao ? instalacaoValue : 0) +
    (includeFrete ? freteValue : 0);

  const billOfMaterials = calculateMaoFrancesaBillOfMaterials({
    height,
    depth,
    quantity,
  });
  ...
  const dimensionControls = [
    ...
    },
    {
      ...
      min: minQuantity,
      max: maxQuantity,
      hint: "Número de mãos-francesas no pedido",
      unit: "un",
    },
  ];
    ...
    <div className="space-y-10 pb-20 lg:pb-0">
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-xl font-bold text-amber-900 dark:text-amber-400 tracking-tight">Dimensões da Mão Francesa</h2>
        </div>
          ...
          <div key={label} className="space-y-4">
            <div className="flex justify-between items-end">
              ...
              <div className="flex items-baseline gap-1">
                ...
                />
                <span className="text-sm font-medium text-amber-900/40 dark:text-amber-500/40">
                  {unit}
                </span>
              </div>
            </div>
            ...
            <div className="pt-6 border-t border-white/10 flex flex-col gap-4">
              ...
              <a
                href={`https://api.whatsapp.com/send?phone=5585992820404&text=${encodeURIComponent(
                  "Olá, gostaria de fazer o pedido de uma mão-francesa de pinus personalizada:\n\n" +
...
export default MaoFrancesaControls;
